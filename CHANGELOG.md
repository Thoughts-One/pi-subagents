...
- **Durable cumulative per-model subagent usage snapshots.** Every terminal top-level `subagents:record` now carries the resolved initial model and a content-free, versioned cumulative usage map keyed by actual provider/model. It records assistant responses, successful compaction calls, nested tool results with explicit `details.usageModel` identity, and nested-agent usage folded into visible ancestors without attaching the same spend to `Agent` or `get_subagent_result` results. Tool usage without explicit model metadata remains in a tool-scoped unattributed bucket instead of inheriting a guessed model. Consumers must select the latest record for each agent ID rather than sum resume revisions; the existing UI total still excludes cache reads.
...
### Fixed
- **Steering, result persistence, and worktree preservation now report verified outcomes.** `AgentManager.steer()` is the awaited typed authority for the top-level tool, nested tool, `/agents` viewer, and FleetView: callers receive accepted, volatile queued, or rejected delivery state, and only accepted delivery emits `subagents:steered`. A failed `subagents:record` append remains visible in the foreground result, completion notification, and live retrieval as a live-only result instead of being silently lost; once the live record is evicted, normal unknown-result behavior remains. Worktree cleanup now verifies the shared Git repository and branch tip before removal, returns canonical `repository`, `branch`, and `commit` fields for preserved work, and retains paths on `wrong_repository` or `branch_mismatch`.
- **Completed background-agent results remain retrievable after live-record eviction.** `get_subagent_result` now falls back only to the latest matching `subagents:record` on the active session branch after its in-memory root record is gone. It validates the durable terminal outcome before rendering its carried fields, refuses malformed or unavailable entries explicitly, and never searches sibling branches or restores an expired agent session.
- **Failed worktree preservation retains unfinished files and reports recovery details.** Inspection, commit, or branch-preservation failures now keep the temporary worktree in place and return the Git operation error instead of force-removing work or reporting no changes. The manager exposes that failure through the terminal status, persisted outcome, result retrieval, and notification while keeping any child output as partial output; a stop read cannot consume the notification before preservation settles, manager-level running and wait checks remain active until a stopped execution settles, recovery messages name the retained temporary path and its system-cleanup risk, and resume is blocked while the preservation failure remains unresolved.
- **Plan authority receipts now derive their exact Claude model identity from the shared schema-v2 contract.** The package rejects old contract schemas and moving or older Fable identities in success and fallback receipts.
